...
Flotr = {
  ...
  drawText: function(ctx, text, x, y, style) {
    ...
    ctx.fillStyle = style.color;
      var normal_fontsize = style.size < 12 ? 12: style.size;
    ctx.font = (style.weight > 1 ? "bold " : "") + (normal_fontsize*1.3) + "px sans-serif";
    ...
    function drawLabelHtml (graph, axis) {
      ...
      if (axis.options.showLabels && (isFirst ? true : axis.used)) {
        for (i = 0; i < axis.ticks.length; ++i) {
          ...
          html += [
            '<div style="position:absolute; font-size:20px;text-align:' + (isX ? 'center' : 'right') + '; ',
            'top:' + top + 'px; ',
