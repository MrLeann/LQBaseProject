...
#import <AVFoundation/AVFoundation.h>//扫描用到的框架


@interface QRCodeScanVC ()<UITabBarDelegate,AVCaptureMetadataOutputObjectsDelegate>{
    
}

// 扫描线顶部的约束
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *vScanLineTopLayout;

//扫描View的高度
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *vScanViewHeightLayout;
...
//扫描线条
@property (weak, nonatomic) IBOutlet UIImageView *vScanLineImg;
//扫描边框图
@property (weak, nonatomic) IBOutlet UIImageView *vScanImg;
...
-(void)mInitUI{
    ...
    //适配语言的
    //self.bar1.title = kLanguages(@"二維碼", @"二维码", @"QR Code");
    //    self.bar2.title = kLanguages(@"條形碼", @"条形码", @"Bar Code");
    //    self.resultLab.text = kLanguages(@"請將二維碼放入上面框中", @"请将二维码放入上面框中", @"Please put the QR code");
...
{
    // 让扫描线条约束 从顶部 0 开始
    self.vScanLineTopLayout.constant = 0;
   
    // 设置动画指定的次数
    [self.view layoutIfNeeded];
    [UIView animateWithDuration:2.0 animations:^{
        
        // 1.修改约束,扫描线到达扫描View的底部
        self.vScanLineTopLayout.constant = self.vScanViewHeightLayout.constant;
...
- (void)tabBar:(UITabBar *)tabBar didSelectItem:(UITabBarItem *)item{
    ...
    if (item.tag == 1) {
        self.vScanViewHeightLayout.constant = 180;
    } else {
        self.vScanViewHeightLayout.constant = 150;
    }
    
    // 2.停止动画
    [self.view.layer removeAllAnimations];
    [self.vScanLineImg.layer removeAllAnimations];//扫描线条停止动画
...
#pragma mark - 返回按钮点击
- (IBAction)mBackButtonClick:(UIButton *)sender {
