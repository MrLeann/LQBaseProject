...
/*
    1、键盘弹出取消自动移动类。
    2、本扩展默认实现了UITextField,UITextView,UISearchBar的键盘弹出机制,要想访问这个类的实例则可以使用UIView的分类中定义的扩展属性：kbMoving。
    3、一般情况下用户不需要任何编码就可以实现键盘的自动弹出。
...
/*
    ...
    1、用于实现具有输入焦点的控件的键盘的弹出和自动隐藏以及控件的自动移动机制。
    2、注意在模拟器中测试可能会出现移动的问题，真机则不会。
